...
if __name__ == "__main__":
    ...
    for k in range(num_runs):
        ...
        for i in range(max_steps):
            ...
            action = RL_step()[2]
            optimal_action[i] = RL_env_message("optimal_action")
            if int(action[0]) == int(optimal_action[i]):
                good[i] = good[i] + 1
            '''
    ...
    for i in range(max_steps):
        print(good[i]/num_runs)

    save_results(good / num_runs, max_steps, "RL_EXP_OUT.dat")
    print "\nDone"
